...
const managePostContacts = require('../src/controllers/sudsconnection/post.js').managePostContacts;
...
describe('tests the managePostContacts function.', function () {
	const testContCn = '1234.123456';
	const contCnPattern = /[0-9]{4}.[0-9]{6}/;
	const token = 'asdf';
	const getApiObj = () => {
		return {
			'GET': {
	...
	const responseFactory = factory.factory({
		'contCn': testContCn,
		'contId': 'MARLOWE, PHILIP',
	...
	const responseOrgFactory = factory.factory({
		'contCn': testContCn,
		'contId': 'UMBRELLA',
		'contName': 'Some corporation',
	...
	async function managePostContactsWrapper(testInfoUpdate, person, response) {
		const apiObj = getApiObj();
		const testInfo = noncommercialFactory.create(testInfoUpdate);
	...
	it('should return contCn for person applications, matching single contact returned by GET on contactGETOptions.requestParams', async () => {
		const person = true;
		const marloweContCn = '4321.654321';
		const response = [responseFactory.create({'contCn': marloweContCn})];
		const result = await managePostContactsWrapper({'applicantInfo': {'firstName': 'Philip', 'lastName': 'Marlowe'}}, person, response);
		expect(result).to.match(contCnPattern);
		expect(result).to.eql(marloweContCn);
	});

	it('should return contCn for person applications, matching one of two contacts returned by GET on contactGETOptions.requestParams', async () => {
		const person = true;
		const marloweContCn = '4321.654321';
		const response = [responseFactory.create({'contId': 'MARLOWE, RAYMOND'}), responseFactory.create({'contCn': marloweContCn})];
		const result = await managePostContactsWrapper({'applicantInfo': {'firstName': 'Philip', 'lastName': 'Marlowe'}}, person, response);
		expect(result).to.match(contCnPattern);
		expect(result).to.eql(marloweContCn);
	});

	it('should return contCn for person applications, creating a new contCn if contIds of responses do not match', async () => {
		const person = true;
		const marloweContCn = '4321.654321';
		const response = [responseFactory.create(), responseFactory.create({'contCn': marloweContCn})];
	...
	it('should throw an error if contacts returned by GET on contactGETOptions.requestParams match', async () => {
		...
		try {
			await managePostContactsWrapper({'applicantInfo': {'firstName': 'Philip', 'lastName': 'Marlowe'}}, person, response);
		}
		catch (error) {
	...
	it('should return contCn for org application, non-matching single contact return by GET on contactGETOptions.requestParams', async () => {
		const person = false;
		const response = [responseOrgFactory.create()];
		const result = await managePostContactsWrapper({}, person, response);
		expect(result).to.match(contCnPattern);
	});

	it('should return contCn for org application, matching single contact returned by GET on contactGETOptions.requestParams', async () => {
		const person = false;
		const umbrellaContCn = '4321.654321';
		const response = [responseOrgFactory.create({'contCn': umbrellaContCn})];
		const result = await managePostContactsWrapper({'applicantInfo': {'organizationName': 'Umbrella'}}, person, response);
		expect(result).to.match(contCnPattern);
		expect(result).to.eql(umbrellaContCn);
	});

	it('should return contCn for org application, matching one of two contacts returned by GET on contactGETOptions.requestParams', async () => {
		const person = false;
		const umbrellaContCn = '4321.654321';
		const response = [responseOrgFactory.create({'contId': 'UMBRELLA', 'contCn': umbrellaContCn}), responseOrgFactory.create({'contId': 'UMBRELLAX'})];
		const result = await managePostContactsWrapper({'applicantInfo': {'organizationName': 'Umbrella'}}, person, response);
		expect(result).to.match(contCnPattern);
		expect(result).to.eql(umbrellaContCn);
	});

	it('should return contCn for org application, creating a new contCn if contIds of responses do not match', async () => {
		const person = false;
		const umbrellaContCn = '4321.654321';
		const response = [responseFactory.create(), responseFactory.create({'contCn': umbrellaContCn})];
		const result = await managePostContactsWrapper({'applicantInfo': {'organizationName': 'Weyland-Yutani'}}, person, response);
		expect(result).to.match(contCnPattern);
		expect(result).to.not.eql(umbrellaContCn);
	});

	it('should throw an error if contacts returned by GET on contactGETOptions.requestParams match', async () => {
		const person = false;
		const response = [responseOrgFactory.create(), responseOrgFactory.create()];
		try {
			await managePostContactsWrapper({'applicantInfo': {'organizationName': 'Umbrella'}}, person, response);
		}
		catch (error) {
			expect(error.name).to.eql('DuplicateContactsError');
			expect(error.message).to.eql('2 duplicate contacts found!');
