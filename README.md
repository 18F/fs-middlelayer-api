# US Forest Service Open Forest Middlelayer API
...
[![CircleCI](https://circleci.com/gh/18F/fs-open-forest-middlelayer-api.svg?style=svg)](https://circleci.com/gh/18F/fs-open-forest-middlelayer-api)
[![Code Climate](https://codeclimate.com/github/18f/fs-open-forest-middlelayer-api/badges/gpa.svg)](https://codeclimate.com/github/18f/fs-open-forest-middlelayer-api)
[![GitHub Tags](https://img.shields.io/github/tag/18f/fs-open-forest-middlelayer-api.svg)](https://github.com/18f/fs-open-forest-middlelayer-apitags)
[![GitHub Contributors](https://img.shields.io/github/contributors/18f/fs-open-forest-middlelayer-api.svg)](https://github.com/18f/fs-open-forest-middlelayer-apigraphs/contributors)
[![GitHub Issues](https://img.shields.io/github/issues/18f/fs-open-forest-middlelayer-api.svg)](https://github.com/18f/fs-open-forest-middlelayer-api)
...
[![Known Vulnerabilities](https://snyk.io/test/github/18f/fs-open-forest-middlelayer-api/badge.svg)](https://snyk.io/test/github/18f/fs-middlelayer-api)

A repository for the development of an API to support the public facing USFS Open Forest system to connect to the related Forest Service database, the Special Use Data System (SUDS) located in the National Resource Management System.
...
5. Extend the schema, if necessary.
...
- `ct t -s <space>`
- `cf run-task <application> "npm run create-admin -- -u <username> -p <password>" --name create-user` 

  Where
  - "space" is one of `middlelayer-dev`, `middlelayer-staging`, `middlelayer-production`
  - "application" is the corresponding `fs-middlelayer-api-dev`, `fs-middlelayer-api-staging`, `fs-middlelayer-api`
...
To emulate the deployed environment variables we provide a list of local `VCAP_SERVICES` that will be automatically pulled in.
...
- `VCAP_SERVICES`: an object to replicate the bound services of the SUDS_API_URL and the S3 bucket. 
  ...
  where `VCAP_SERVICES` is provided in this [format](https://github.com/18F/fs-cloud-gov-migration/#format) for the format of each set of credentials.

  | Name | Credentials | Dev values |
  | - | - | - |
  | `new-relic` | `key: <key>, app_name: <app_name>` | `key = ""` |
  | `auth-service` | `JWT_SECRET_KEY: <secret>` | `secret = "SECRET"`|
  | `nrm-suds-url-service` | `SUDS_API_URL: <suds_url>, username: <username>, password: <password>` | `suds_url = "MOCKS"` |
